<template>
  <div class="add-liste">
    ...
    <template v-if="!show_recap">
      ...
      <b-form class="form-add">
        <b-form-group
          class="animate__animated animate__fadeInRight position-relative"
          style="z-index: 1"
          label="A quelle date souhaitez-vous faire vos courses ?"
        >
        </b-form-group>
        <b-form-datepicker
        ...
        >
          <b-row>
            ...
            <b-col
              ...
              class="animate__animated animate__zoomIn text-right"
              :class="'text-' + status_color"
            >
              <span class="total-restant"
                >Total TTC :
                <strong
                  >{{ parseFloat(animatedTTC).toFixed(2) }} &#8364;</strong
                >
              </span>
            </b-col>
          </b-row>
      ...
      this.$axios
        ...
        .then((res) => {
          this.$store.dispatch("ADD_STATS").then((resp) => {
            ...
            _this.$bvToast.toast("Votre liste a bien été ajoutée", {
              title: "Ajout réussie",
