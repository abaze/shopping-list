      ...
      <b-col class="p-3">
        ...
        >
          <h5 class="p-3">
            ...
            {{
              all_listes
                ? `${new Date(all_listes[0].date).getDate()}/${new Date(
                    all_listes[0].date
                  ).getMonth() + 1}/${new Date(
                    all_listes[0].date
                  ).getFullYear()}`
      ...
      if (this.all_listes) {
        return {
          labels: this.all_listes.map(
            stat =>
              `${new Date(stat.date).getDate()}/${new Date(
                stat.date
              ).getMonth() + 1}/${new Date(stat.date).getFullYear()}`
          ),
          datasets: [
            {
        ...
        this.all_listes.forEach(function(liste) {
          liste.produits.forEach(function(cat) {
            cat.produits.forEach(function(p, i) {
              const price = _this.all_products.find(pr => pr.id === p.id).price;
              const photo = _this.all_products.find(pr => pr.id === p.id).image;
              ...
              if (!produits_buy.find(article => article.id === p.id)) {
                ...
                produits_buy.find(prod => prod.id === p.id).prix += parseFloat(
                  quantity * price * process.env.TVA
                );
              }
            });
          });
  ...
  methods: {
    initLineChart() {
      ...
      this.data_chart.labels = this.all_listes.map(
        stat =>
          `${new Date(stat.date).getDate()}/${new Date(stat.date).getMonth() +
            1}/${new Date(stat.date).getFullYear()}`
      );
    ...
    initDoughnutChart() {
      ...
      this.data_doughnut.datasets[0].backgroundColor = this.data_depense.depense_cat.map(
        cat => cat.color
      );
